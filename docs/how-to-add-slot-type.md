...
// public bool IsDurationEditable => SelectedPreset is not (LoopBeginPreset or LoopEndPreset or BreakpointPreset or PlaySoundPreset);
```
...
<StackPanel Visibility="{Binding SelectedPreset.IsPlaySound, Converter={StaticResource BoolToVis}}">
    ...
    <StackPanel Orientation="Horizontal">
        ...
        <TextBox Text="{Binding SoundVolume, UpdateSourceTrigger=LostFocus}"
                 ...
                 b:NumericTextBoxBehavior.MinValue="0"
                 b:NumericTextBoxBehavior.MaxValue="1" />
    </StackPanel>
</StackPanel>
```
...
### 5-1. durationMs の switch 式に追加する
...
int durationMs = slot switch
{
    ...
    PlaySoundSlot ps   => ps.DurationMs,  // ← 追加
    _ => 0,
};
```
...
- [ ] `SequencePlayerService.cs` — `durationMs` の switch 式に追加した
...
| コンパイルエラー `CS8509` (switch 網羅性)             | `ToModel()` または `FromModel()` の switch に case が足りない                        |
