...
| フィールド   | 型      | 説明                                            |
| ------------ | ------- | ----------------------------------------------- |
| `button`     | integer | ボタン種別（下表）                              |
| `action`     | integer | キーアクション（key_single の action と同じ値） |
| `durationMs` | integer | 待機時間（ms）                                  |
...
| 値  | 意味                 | 座標系                                 |
| --- | -------------------- | -------------------------------------- |
| 0   | Relative（相対移動） | ピクセル単位の差分                     |
| 1   | Absolute（絶対移動） | InputSettings.MouseMode に依存（後述） |
